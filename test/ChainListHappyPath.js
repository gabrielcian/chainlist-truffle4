...
contract('ChainList', function(accounts) {
  ...
  var articleDescription2 = "Description for article 1";
  ...
  it("should be initialized with empty values", function() {
      return ChainList.deployed().then(function(instance){
        ...
        assert.equal(data.length, 0, "there shouldn t be any article to sell");
      });
  });
  ...
  it("should let us sell a first article", function(){
    return ChainList.deployed().then(function(instance){
      ...
      assert.equal(receipt.logs.length, 1, "only one event is supposed to be triggered");
      assert.equal(receipt.logs[0].event, "LogSellArticle", "event should be LogSellArticle");
      assert.equal(receipt.logs[0].args._id.toNumber(), 1, "_id must be 1");
      assert.equal(receipt.logs[0].args._seller, seller, "event seller must be " + seller);
      assert.equal(receipt.logs[0].args._name, articleName1, "event article must be " + articleName1 + " but is equal to " + receipt.logs[0].event._name);
  ...
  it("should let us sell a second article", function(){
    return ChainList.deployed().then(function(instance){
      ...
      assert.equal(receipt.logs[0].args._seller, seller, "event seller must be " + seller);
      assert.equal(receipt.logs[0].args._name, articleName2, "event article must be " + articleName2 + " but is equal to " + receipt.logs[0].event._name);
      assert.equal(receipt.logs[0].args._price.toNumber(), web3.toWei(articlePrice2, "ether"), "event price should be " + web3.toWei(articlePrice2, "ether"));
  ...
  it("should buy an article", function() {
    return ChainList.deployed().then(function(instance) {
      ...
      //check the effect of the buyer and seller, accounting for gas
      assert(sellerBalanceAfterBuy == sellerBalanceBeforeBuy + articlePrice1, "seller should have earned " + articlePrice1 + " ether but sellerBalanceAfterBuy = " + sellerBalanceAfterBuy + ", sellerBalanceBeforeBuy = " + sellerBalanceBeforeBuy + " and articlePrice = " + articlePrice1);
      assert(buyerBalanceAfterBuy <= buyerBalanceBeforeBuy - articlePrice1, "buyer should have spent " + articlePrice1 + " ether");
      return chainListInstance.getArticlesForSale();
    }).then(function(data) {
      assert.equal(data.length, 1, "there should be now only 1 articles left for sale");
